...
>
	...
	<PageHeader title={`Welcome ${userName}!`}>
		<div class="flex w-full flex-col items-center gap-4 py-1.5 md:flex-row">
			<div
				class="mt-4 flex items-center gap-2 rounded-full bg-white px-6 py-2 text-sm font-medium shadow transition-transform duration-200 hover:scale-105 md:mt-0"
			>
	...
	<section class="container mx-auto p-4">
		...
		<div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
			<FormManageinventory {products} onUpdate={handleUpdateInventory} />

			<TopProductsTable
				products={productsUnderMinStock.map((p) => ({
					...p,
