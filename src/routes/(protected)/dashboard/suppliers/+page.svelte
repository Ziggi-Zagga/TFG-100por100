...
>
	<!-- HEADER & SEARCHBAR -->
	<Header title="Suppliers" subtitle={totalUsers().toString() + ' suppliers'} />

	<div class="mb-1 flex flex-col items-center gap-4 md:flex-row">
		<div class="w-full md:flex-1">
			<SearchBar bind:search={search} placeholder="Search by name, email, contact..." />
		</div>
	</div>
	...
	{#if showDrawer}
		<Drawer title="Add New Supplier" onClose={closeDrawer}>
			<h2 class="mb-4 text-2xl font-bold">Add New Supplier</h2>
			<form
				method="POST"
				use:enhance
			...
			>
				<div>
					...
					<TextInput name="name" placeholder="Enter supplier name" extraStyles="w-full" />
				</div>

				<div>
					<label class="font-semibold">Email</label>
					<TextInput name="email" placeholder="Enter supplier email" extraStyles="w-full" />
				</div>
