<script lang="ts">
  ...
  const { gapName = '', onClose = () => {}, class: className = '' } = $props<{
    gapName: string;
    onClose: () => void;
    class?: string;
  }>();
...
<div class="flex items-center {className}">
  ...
  <div 
    class="flex items-center bg-white border border-gray-200 shadow-sm rounded-full overflow-hidden h-10"
    style="width: {isExpanded ? 'auto' : '2.5rem'}; padding: 0 {isExpanded ? '0.75rem' : '0'}; gap: {isExpanded ? '0.5rem' : '0'}"
    in:slide|local={{ duration: 300, easing: quadInOut, axis: 'x' }}
    out:slide|local={{ duration: 250, easing: quadInOut, axis: 'x' }}
  >
    <!-- Ícono -->
    <div class="flex-shrink-0 flex items-center -mr-2 justify-center w-12 h-12">
      {#if isExpanded}
