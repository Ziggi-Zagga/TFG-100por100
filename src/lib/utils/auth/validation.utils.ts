...
export class ValidationUtils {
  static validatePassword(password: string): void {
    if (password.length < AUTH_CONSTANTS.PASSWORD.MIN_LENGTH || password.length > AUTH_CONSTANTS.PASSWORD.MAX_LENGTH) {
  ...
  static validateUsername(username: string): void {
    if (username.length < AUTH_CONSTANTS.USERNAME.MIN_LENGTH || username.length > AUTH_CONSTANTS.USERNAME.MAX_LENGTH) {
      throw new ServiceError(
  ...
  static validateEmail(email: string): void {
    if (!EMAIL_PATTERN.test(email)) {
      throw new ServiceError('Invalid email format', ERROR_TYPES.VALIDATION, 400, { field: 'email' });
    }
  }

  static validateFullName(fullName: string): void {
    if (!FULLNAME_PATTERN.test(fullName)) {
      throw new ServiceError('Invalid full name format', ERROR_TYPES.VALIDATION, 400, { field: 'fullName' });
    }
  }
