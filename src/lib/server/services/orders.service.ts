...
export const createOrderWithItems = async (orderData: {
	...
	const nextOrderNumber = lastOrderNumber ? Number(lastOrderNumber.replace('ORD-', '')) + 1 : 1;
	...
	try {
		await db.transaction(async (tx: any) => {
			// Using any for tx as a quick fix, but should be properly typed in production
